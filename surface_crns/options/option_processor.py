    ...
    def update_colormap(self, state):
        if not state in self.COLORMAP:
            ...
            if self.n_colors >= 50:
                ...
                new_color = rcolor.generate_new_color(self.colormap_values,0)
                self.COLORMAP[state] = new_color
                self.colormap_names.add(state)
                self.colormap_values.add(tuple(new_color))
